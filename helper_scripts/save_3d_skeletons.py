...
def initialize_detector():
    ...
    options = PoseLandmarkerOptions(base_options=base_options, output_segmentation_masks=False)
...
def main(base_path):
    num_cores_to_use = 50 #min(8, os.cpu_count())  # Use a reasonable number of CPU cores

    with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores_to_use) as executor:
        futures = []
        ...
        for root, dirs, files in os.walk(base_path):
            if 'calib' in root:
                ...
                images = [os.path.join(rgb_folder, i) for i in os.listdir(rgb_folder) if i.endswith(".jpg")]
                ...
                for image_path in images:
                    futures.append(executor.submit(process_image, image_path, output_folder))
                ...
                dirs.clear()
                print(f"Finished processing folder: {root}")
