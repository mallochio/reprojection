...
def project_kinect_to_omni(frames, k_idx, fk):
    ...
    world_coordinates, valid_depth_coordinates = kinect.depth_to_world(masked_depth, depth,
                                                                       fk.kinect_params[k_idx]['k_params']) # k_params is (fx, fy, cx, cy) from intrinsics matrix
...
def main():
    ...
    side_out = cv2.VideoWriter(
        'out/output_side.avi', fourcc, 15.0, side_vid_size)
    ...
    for ts in tqdm(fk.get_lead_span()):
        ...
        for k_idx in range(fk.num_kinects):
            Xu, Yu, depth_vis, binary_mask = project_kinect_to_omni(
                frames, k_idx, fk)
            omni_image[Yu, Xu] = omni_image[Yu, Xu] / \
                1.5 + np.array(assigned_colors[k_idx])
            omni_mask[Yu, Xu] = 255
            ...
            out_frame[vid_size[1]//2:, k_idx*dsize[0]:k_idx*dsize[0]+dsize[0], :] =\
                cv2.cvtColor(cv2.resize(binary_mask, dsize),
                             cv2.COLOR_GRAY2BGR)

        # cv2.imshow('fisheye view', cv2.resize(omni_image, dsize=dsize))
        out_frame[0:vid_size[1]//2, 2*dsize[0]:,
                  :] = cv2.resize(omni_image, dsize)
        ...
        out_frame[vid_size[1]//2:, 2*dsize[0]:,
                  :] = cv2.cvtColor(cv2.resize(omni_mask, dsize), cv2.COLOR_GRAY2BGR)
        side_frame[:, uw:] = cv2.cvtColor(cv2.resize(
            omni_mask, (uw, uh)), cv2.COLOR_GRAY2BGR)
